...
for i =1:size_in
    if(bus_branch)
        data_out.p(i, :) = [Va_in(i,:) * pi / 180 V_in(i,:).^2 1] * Xp';
        data_out.q(i, :) = [Va_in(i,:) * pi / 180 V_in(i,:).^2 1]*Xq';
    else
        data_out.pf(i, :) = [Va_in(i,:) * pi / 180 V_in(i,:).^2 1] * Xpf';
        data_out.qf(i, :) = [Va_in(i,:) * pi / 180 V_in(i,:).^2 1] * Xqf';
    end
end
