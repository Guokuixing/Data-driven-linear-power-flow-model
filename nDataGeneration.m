function [ntest] = ...
    nDataGeneration(case_name, Q_per, data_name, dc_ac, G_range, ...
    upper_bound, lower_bound, Q_range, V_range, L_range, ...
    random_load, Va_range, ref, L_corr, ntset)
...
define_constants;
    ...
    A1= mpc.branch(:,11);
    data_size = 1;
    data.P = zeros(num_branch, num_load);
    data.Va = zeros(num_branch, num_load);
    if(dc_ac)
        data.Q = zeros(num_branch, num_load);
        data.V = zeros(num_branch, num_load);
        data.Va_dc = zeros(num_branch, num_load);
        data.P_dc = zeros(num_branch, num_load);
    end
...
%%  generate the data based on AC power flow equations or based on DC power flow equations
    ...
    ntest.p.dcpf(i, :) = B * data.Va(i, :)' * pi / 180;
    ntest.p.dlpf(i, :) = [data.Va(i,:) * pi / 180 data.V(i,:)]*Xp_dlpf';
    ntest.q.dlpf(i, :) = [data.Va(i,:) * pi / 180 data.V(i,:)]*Xq_dlpf';
end
%% save to files
