...
for i = 1:num_train
    test.p.fitting(i, :) = [data.Va(i,:) * pi / 180 data.V(i,:).^2 1] * Xp';
    test.p.dcpf(i, :) = B * data.Va(i, :)' * pi / 180;
    test.p.dlpf(i, :) = [data.Va(i,:) * pi / 180 data.V(i,:)]*Xp_dlpf';
    test.q.fitting(i, :) = [data.Va(i,:) * pi / 180 data.V(i,:).^2 1]*Xq';
